...
export default function Navbar({ onMenuClick }) {
  ...
  return (
    <nav className="h-24 bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40 px-6 lg:px-10 flex items-center justify-between">
      {/* Search & Intelligence */}
      <div className="flex items-center gap-4 lg:gap-6 flex-1 max-w-xl">
        {/* Hamburger Menu (Mobile Only) */}
        <button 
          onClick={onMenuClick}
          className="lg:hidden p-3 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-all text-slate-500"
        >
      ...
      <div className="flex items-center gap-4 lg:gap-8 ml-4">
        ...
        {/* Notifications */}
        <button className="relative p-3 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-all group hidden sm:block">
        ...
        <div className="flex items-center gap-4 pl-4 lg:pl-8 border-l border-slate-100">
          <div className="text-right hidden md:block">
            ...
            <p className="text-[11px] font-bold text-slate-400 mt-1 uppercase tracking-widest truncate max-w-[120px]">{user?.businessName || 'Vyapar Flow'}</p>
          </div>
          <motion.div 
            whileHover={{ scale: 1.05 }}
            className="w-10 h-10 lg:w-12 lg:h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 p-[2px] shadow-lg shadow-indigo-500/20"
          >
            <div className="w-full h-full rounded-[14px] bg-white flex items-center justify-center">
              <span className="text-indigo-600 font-black text-base lg:text-lg">
