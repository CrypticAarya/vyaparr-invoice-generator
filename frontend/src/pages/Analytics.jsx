    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard 
          ...
          value={`₹${((metrics.totalRevenue || 0) / Math.max((analyticsData.recentActivity?.length || 0), 1)).toLocaleString(undefined, {maximumFractionDigits: 0})}`} 
          icon={ARPathIcon}
          color="purple"
        />
        <StatCard 
          ...
          subValue={`${metrics.outOfStockCount || 0} Out of stock`}
          icon={ProductIcon}
          color="amber"
        />
        <StatCard 
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={charts.topClients}>
              ...
              <Tooltip 
                contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)', fontWeight: 'bold' }}
              />
              <Bar dataKey="revenue" fill="#818cf8" radius={[8, 8, 0, 0]} barSize={40} data={charts.topClients || []} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
